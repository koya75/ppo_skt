...
class Franka:
    def create(self, gym, sim, device, discrete, actionRepeat, asset_root):
        ...
        self.target_hand_pos_upper_limits = to_torch(
            [0.15, 0.5, 1.0], device=self.device
        )
    ...
    def set_tensors(self, rigid_body_states, num_envs, dof_state, sim_params, viewer):
        ...
        self.target_hand_pos_offset[:, 2] = 0.52
    ...
    def pre_physics_step(self, actions, item_pos):
        ...
        if self.discrete:
            ...
            one_hot = torch.nn.functional.one_hot(actions.to(torch.int64), num_classes=9).to(
                torch.float
            )
            dx = to_torch([0, -dv, dv, 0, 0, 0, 0, 0, 0], device=self.device,).inner(one_hot)
            dy = to_torch([0, 0, 0, -dv, dv, 0, 0, 0, 0], device=self.device,).inner(one_hot)
            dz = to_torch([0, 0, 0, 0, 0, -dv, dv, 0, 0], device=self.device,).inner(one_hot)
            da = to_torch([0, 0, 0, 0, 0, 0, 0, -10 * dv, 10 * dv], device=self.device,).inner(one_hot)
        else:
            dx = dv * actions[:, 0]
            dy = dv * actions[:, 1]
    ...
    def _step_continuous(self, dtarget_hand_pos, dtarget_hand_rot_z):
        ...
        for env in range(self.num_envs):
            ...
            if count > 0 and torch.round(self.target_hand_pos[env][1], decimals=2) > 0.3:
                self.envs.remove(env)
            else:# self.target_hand_pos[env][1] <= 0.3
                for i in range(2):
                    if distance[env,i] > 0.04 and distance[env,i] < 0.08:
