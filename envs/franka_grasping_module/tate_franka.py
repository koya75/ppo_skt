...
class Franka:
    def create(self, gym, sim, device, discrete, actionRepeat, asset_root, max_episode_length):
        ...
        self.default_dof_pos = to_torch(
            [0, 0.0, 0, -2.0, -2.8973, 2.75, 0.0, 0.035, 0.035], device=self.device
        )
        ...
        self.target_hand_rot_x_lower_limits = to_torch([-2.0], device=self.device)
        self.target_hand_rot_x_upper_limits = to_torch([2.0], device=self.device)
        self.target_hand_rot_y_lower_limits = to_torch([-2.0], device=self.device)
        self.target_hand_rot_y_upper_limits = to_torch([2.0], device=self.device)
        self.target_hand_rot_z_lower_limits = to_torch([-4.5], device=self.device)
        self.target_hand_rot_z_upper_limits = to_torch([1.5], device=self.device)
    ...
    def set_tensors(self, rigid_body_states, num_envs, dof_state, sim_params, viewer):
        ...
        self.target_hand_rot_offset[:, 2] = 1
    ...
    def reset(self, env_ids):
        ...
        self.target_hand_rot_y[env_ids] = 1.5
        self.target_hand_rot_z[env_ids] = 1.5

        self.envs = []
        self.t = 0

    def pre_physics_step(self, actions, item_pos):
        ...
        if self.discrete:
            ...
            #da = to_torch([0, 0, 0, 0, 0, 0, 0, -dv, dv], device=self.device,).inner(one_hot)
        ...
        """dtarget_hand_rot_x = dr
        dtarget_hand_rot_y = dp
        dtarget_hand_rot_z = da"""

        done = self._step_continuous(dtarget_hand_pos, item_pos)#, dtarget_hand_rot_x, dtarget_hand_rot_y, dtarget_hand_rot_z
        return done

    def _step_continuous(self, dtarget_hand_pos, item_pos):#, dtarget_hand_rot_x, dtarget_hand_rot_y, dtarget_hand_rot_z
        # Perform commanded action.
        finger = 0.04 * self.ones

        dpos = (item_pos - self.target_hand_pos.unsqueeze(1))
        ...
        for env in range(self.num_envs):
            if torch.round(distance[env], decimals=2) < 0.1:
        ...
        self._apply_action(dtarget_hand_pos, finger)#, dtarget_hand_rot_x, dtarget_hand_rot_y, dtarget_hand_rot_z
    ...
    def _apply_action(self, dtarget_hand_pos, finger):#, dtarget_hand_rot_x, dtarget_hand_rot_y, dtarget_hand_rot_z
        ...
        """self.target_hand_rot_x = self.target_hand_rot_x + dtarget_hand_rot_x
        self.target_hand_rot_x = tensor_clamp(
            self.target_hand_rot_x,
            self.target_hand_rot_x_lower_limits,
            self.target_hand_rot_x_upper_limits,
        )
        self.target_hand_rot_y = self.target_hand_rot_y + dtarget_hand_rot_y
        self.target_hand_rot_y = tensor_clamp(
            self.target_hand_rot_y,
            self.target_hand_rot_y_lower_limits,
            self.target_hand_rot_y_upper_limits,
        )
