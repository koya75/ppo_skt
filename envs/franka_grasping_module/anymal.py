...
class Franka:
    def create(self, cfg, gym, sim, device, discrete, actionRepeat, asset_root, episode_length):
        self.cfg = cfg
        self.gym = gym
        self.sim = sim
        self.device = device
        ...
        for i in range(self._num_dofs):
            ...
            self.dof_props["stiffness"][i] = self.cfg["env"]["control"]["stiffness"] #self.Kp
            self.dof_props["damping"][i] = self.cfg["env"]["control"]["damping"] #self.Kd
    ...
    def set_tensors(self, rigid_body_states, num_envs, dof_state, sim_params, viewer):
        ...
        self.target_hand_pos_offset[:, 1] = 0.2
        self.target_hand_pos_offset[:, 2] = 0.53
    ...
    def reset(self, env_ids, env_ids_int32):
        # frankaの初期位置を再設定する
        # 初期位置を若干ランダムに設定する。
        """pos = tensor_clamp(
        ...
        self.target_hand_pos[env_ids, 2] = 0.0
        self.target_hand_rot_z[env_ids] = 1.5

        self.envs = []
        self.t = 0

    def pre_physics_step(self, actions, n_actions):#, item_pos
        #self.item_pos = item_pos

        dv = self._dv  # velocity per physics step.
        if self.discrete:
            # compute position and orientation error
            one_hot = torch.nn.functional.one_hot(actions.to(torch.int64), num_classes=n_actions).to(
                torch.float
            )
            dx = to_torch([0, -dv, dv, 0, 0], device=self.device,).inner(one_hot)
            dy = to_torch([0, 0, 0, -dv, dv], device=self.device,).inner(one_hot)
            dz = torch.zeros_like(dx)
            da = torch.zeros_like(dx)
        else:
            dx = dv * actions[:, 0]
            dy = dv * actions[:, 1]
            dz = torch.zeros_like(dx)
    ...
    def _step_continuous(self, dtarget_hand_pos, dtarget_hand_rot_z):
        ...
        """if self.t <= 4:
            dtarget_hand_pos[:, 0] = self._dv
            dtarget_hand_pos[:, 1] = 0
        else:
            dtarget_hand_pos[:, 0] = 0
            dtarget_hand_pos[:, 1] = self._dv"""
