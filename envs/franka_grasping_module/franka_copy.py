...
class Franka:
    def create(self, gym, sim, device, discrete, actionRepeat, asset_root):
        ...
        self.start_pose.p = gymapi.Vec3(0.0, -0.1, 0.4)
    ...
    def set_tensors(self, rigid_body_states, num_envs, dof_state, sim_params, viewer):
        ...
        self.target_hand_pos_offset[:, 1] = 0.3
        self.target_hand_pos_offset[:, 2] = 0.5

        self.item_pos_offset = torch.zeros_like(self.hand_pos)
        self.item_pos_offset[:, 1] = 0.3
        self.item_pos_offset[:, 2] = 0.42
    ...
    def reset(self, env_ids):
        ...
        self.target_hand_pos[env_ids, 0] = -0.1
        self.target_hand_pos[env_ids, 1] = -0.05
        self.target_hand_pos[env_ids, 2] = 0.3

        self.target_hand_pos2[env_ids, 0] = 0.0
        self.target_hand_pos2[env_ids, 1] = 0.0
        self.target_hand_pos2[env_ids, 2] = 0.0

        self.target_hand_rot_z[env_ids] = 1.8

        self.envs = []

    def pre_physics_step(self, actions, item_pos, n_actions):
        self.item_pos = item_pos - self.item_pos_offset.unsqueeze(1)

        dv = 0.025  # velocity per physics step.
        if self.discrete:
            ...
            dx = dv * actions[:, 0]
            dy = dv * actions[:, 1]
            dz = -self._dv * torch.ones_like(actions[:, 1])
            da = 10 * dv * actions[:, 2]
    ...
    def _step_continuous(self, dtarget_hand_pos, dtarget_hand_rot_z):
        ...
        for t in zip(self.envs):
            ...
            dtarget_hand_pos[t, 0] = 0
            dtarget_hand_pos[t, 1] = 0.025
            dtarget_hand_pos[t, 2] = self._dv
        ...
        dpos = (self.item_pos - self.target_hand_pos.unsqueeze(1))
        distance = torch.norm(dpos,dim=-1)

        done = False
        for env in range(self.num_envs):
            count = self.envs.count(env)
            if count > 0 and torch.round(self.target_hand_pos[env][1], decimals=2) > 0.25:
                self.envs.remove(env)
            else:# self.target_hand_pos[env][1] <= 0.3
                if torch.round(self.target_hand_pos[env][2], decimals=2) < 0.001:#distance[env,i] > 0.04 and distance[env,i] < 0.08:
