...
class Item:
    def create(self, gym, sim, device, num_actors, asset_root, item_names, steps, usedrop, target):
        self.gym = gym
        self.sim = sim
        self.device = device
        ...
        self.initial_pose.p = gymapi.Vec3(0, 0.4, 0.7)
    ...
    def _random_pose(self):
        ...
        pose.p.x = self.initial_pose.p.x - 0.08 + 0.2 * np.random.rand()
        pose.p.y = self.initial_pose.p.y - 0.08 + 0.2 * np.random.rand()
        pose.p.z = self.initial_pose.p.z + 0.2 * np.random.rand()
