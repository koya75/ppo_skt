...
from repos.pfrl.pfrl import agent
from repos.pfrl.pfrl.action_value import ActionValue
from repos.pfrl.pfrl.explorer import Explorer
from repos.pfrl.pfrl.replay_buffer import (
    AbstractEpisodicReplayBuffer,
    ReplayUpdater,
    batch_experiences,
    batch_recurrent_experiences,
)
from repos.pfrl.pfrl.replay_buffers import PrioritizedReplayBuffer
from repos.pfrl.pfrl.utils.batch_states import batch_states
from repos.pfrl.pfrl.utils.contexts import evaluating
from repos.pfrl.pfrl.utils.copy_param import synchronize_parameters
...
class AQT(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def __init__(
        ...
        replay_buffer: pf.replay_buffer.AbstractReplayBuffer,
        gamma: float,
        explorer: Explorer,
    ...
    def update(
        ...
        if self.max_grad_norm is not None:
            pf.utils.clip_l2_grad_norm_(self.model.parameters(), self.max_grad_norm)
        self.optimizer.step()
        self.optim_t += 1
    ...
    def batch_act(self, batch_obs: Sequence[Any], demo_flag=False) -> Sequence[Any]:
    ...
    def setup_actor_learner_training(
        ...
        def make_actor(i):
            return pf.agents.StateQFunctionActor(
                pipe=actor_pipes[i],
                model=shared_model,
        ...
        poller = pf.utils.StoppableThread(
        ...
        learner = pf.utils.StoppableThread(
