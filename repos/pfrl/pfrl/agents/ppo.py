...
from repos.pfrl.pfrl.utils.mode_of_distribution import mode_of_distribution
...
def _add_log_prob_and_value_to_episodes_recurrent(
    ...
    with torch.no_grad(), pf.utils.evaluating(model):
...
class PPO(agent.AttributeSavingMixin, agent.BatchAgent):
    ...
    def _update_once_recurrent(self, episodes, mean_advs, std_advs):
        ...
        with torch.no_grad(), pf.utils.evaluating(self.model):
    ...
    def batch_act(self, batch_obs, demo=False):
        self.demo = demo
        if self.training:
